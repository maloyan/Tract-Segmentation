...
def main(cfg):
    ...
    for epoch in range(cfg.epochs):
        ...
        if (
            (epoch + 1) % cfg.eval_epochs == 0
            and cfg.eval is True
            ...
            if val_metric > best_val_metric:
                ...
                if cfg.load_best_weights is True:
                    try:
                        ...
                        print(
                            f"metric no improve, load the saved best weights with score: {best_val_metric}."
                        )
                    except:
                        pass
...
def run_org_eval(
    ...
    with torch.no_grad():
        for itr in progress_bar:
            ...
            val_outputs = (
                torch.stack(val_outputs).permute([0, 4, 1, 2, 3]).flatten(0, 1)
            )
            val_masks = []
